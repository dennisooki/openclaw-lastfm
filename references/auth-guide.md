...
```bash
curl "http://ws.audioscrobbler.com/2.0/?method=auth.gettoken&api_key=YOUR_API_KEY&format=json"
```
...
1. Construct authorization URL:
   ```
   http://www.last.fm/api/auth/?api_key=YOUR_API_KEY&token=YOUR_TOKEN
   ```
...
# Request session key
curl "http://ws.audioscrobbler.com/2.0/?method=auth.getsession&api_key=${API_KEY}&token=${TOKEN}&api_sig=${SIGNATURE}&format=json"
```
